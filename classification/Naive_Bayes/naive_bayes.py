...
class NAIVE:
    ...
    def modeling_data(self,x_train, x_test, y_train, y_test):
        try:
            self.gau.fit(x_train, y_train)
            y_train_predict = self.gau.predict(x_train)
            y_test_predict = self.gau.predict(x_test)
            print(f'the Training accuracy:{self.gau.score(x_train, y_train)}')
            print(f'the Testing accuracy:{self.gau.score(x_test, y_test)}')
            # Validation Report
...
if __name__=='__main__':
    obj=NAIVE("T:/pycharm/ML/supervisied_model/breast-cancer.csv")# load the dataset
    obj.dataprocessing()